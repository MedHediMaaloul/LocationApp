...
$query = "SELECT FC.id_facture,FC.id_client,FC.date_facture,FC.id_contrat,C.date_contrat,C.mode_de_paiement,C.prix,CL.id_client,CL.nom,CL.cin,CL.email,CL.tel,CL.adresse,A.id_agence,A.lieu_agence,A.email_agence,A.tel_agence,MC.id,MC.designation_contrat,MC.code_materiel_contrat,MC.num_serie_contrat
...
$list_composant = [];
$query1 = " SELECT CM.designation_composant,CM.num_serie_composant
            FROM composant_materiels_contrat AS CM
...
if (count($list_composant[0]) > 15){
  $nblist = 15;
  $listinitial = 0;
  $compteur = intval(count($list_composant[0]) /$nblist) + 1;
  ...
  $datas = array();
  $datas[] = array($Client_id,'DL '.$Facture_number,$Facture_date);
  ...
  if ($nblist > count($list_composant[0])){
      $nblist = count($list_composant[0]);
  }
  // echo $nblist;
  // echo "Hi";
  // echo $listinitial;
  ...
  $datas[] = array($list_composant[0][$i],utf8_decode($list_composant[1][$i]),"1");
  }
  $listinitial = $nblist;
  $nblist = $nblist + 15;
  $prixTotalHorsTaxes=150;
  $prixTVA = $prixTotalHorsTaxes * ($Devis_tva/100);
  //Tableau contenant les titres des colonnes
  $header=array(utf8_decode('Référence'),utf8_decode('Désignation'),utf8_decode('Qté'));
  ...
  for ($i=0; $i < count($list_composant[0]); $i++) {
    $datas[] = array(" ",$list_composant[1][$i]." - ".$list_composant[0][$i]," ",' '," "," "," ");
  }
  $prixTotalHorsTaxes=$Contrat_prix;
  $Devis_tva=20;
  $prixTVA = $prixTotalHorsTaxes*($Devis_tva/100);
